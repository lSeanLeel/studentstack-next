  ...
  return (
    <div>
      ...
      <p className={`mt-2 max-w-2xl text-sm font-medium text-slate-600 sm:text-base ${jakartaSans.className}`}>
        Full access to the workflows we use for school, organized by category. Content here is designed to stay current
        as tools change.
      </p>
            ...
            <ul className="mt-4 grid gap-3 sm:grid-cols-2">
              {category.tools.map((tool) => (
                <li key={tool.id} className="rounded-[1.5rem] border border-slate-100 bg-white p-5">
                  <p className={`text-lg font-semibold text-slate-900 ${fredokaHeadline.className}`}>{tool.name}</p>
                  <p className={`mt-1.5 text-sm font-medium leading-relaxed text-slate-600 ${jakartaSans.className}`}>
                    {tool.blurb}
                  </p>
                  <p className="mt-3 text-[11px] font-black uppercase tracking-[0.14em] text-sky-600">

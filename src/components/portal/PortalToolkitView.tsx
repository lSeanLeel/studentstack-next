  ...
  return (
    <div className="space-y-8">
      ...
      <PortalPanel tinted>
        ...
        <p className={`mt-2 max-w-3xl text-sm font-medium leading-relaxed text-slate-500 ${jakartaSans.className}`}>
          {tip.body}
        </p>
        {tipTool && tipCategory ? (
          <a
            href={`#${tip.categoryId}`}
            className={`mt-3 inline-flex items-center gap-2 text-sm font-semibold text-sky-600 hover:text-sky-700 ${jakartaSans.className}`}
          >
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_minmax(18rem,22rem)] lg:items-start">
        <div>
          <label className={portalLabel}>Filter by category</label>
          <div className="relative mt-2 max-w-xs">
            <select
              value={filter}
              onChange={(e) => setFilter(e.target.value as FilterId)}
              className={`w-full appearance-none rounded-2xl bg-[#f5f5f7] py-2.5 pl-4 pr-10 text-sm font-semibold text-slate-800 ring-1 ring-black/[0.06] focus:bg-white focus:outline-none focus:ring-sky-500/30 ${jakartaSans.className}`}
            >
          ...
          <div className="mt-6 space-y-10">
            {categories.map((category) => (
              <section key={category.id} id={category.id} className="scroll-mt-32">
                ...
                <p className={`mt-1 text-sm font-medium text-slate-500 ${jakartaSans.className}`}>{category.summary}</p>
                <ul className="mt-4 space-y-4">
                  {category.tools.map((tool) => {
                    ...
                    return (
                      <li
                        key={tool.id}
                        className={`${portalCard} p-5 ${featured ? "ring-sky-300/60" : ""}`}
                      >
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          <div>
                            <p
                              className={`text-xs font-semibold ${featured ? "text-sky-600" : "text-slate-400"} ${jakartaSans.className}`}
                            >
                          ...
                          <p className={`text-xs font-semibold text-orange-600 ${jakartaSans.className}`}>
                            {tool.useCase}
                          </p>
                        </div>
                        ...
                        <div className="mt-4">
                          <p className={`text-xs font-semibold text-slate-400 ${jakartaSans.className}`}>How we use it</p>
                          <p className={`mt-1 text-sm font-medium leading-relaxed text-slate-700 ${jakartaSans.className}`}>
                            {tool.howWeUse}
                          </p>
                        </div>

                        <div className="mt-4">
                          <p className={`text-xs font-semibold text-slate-400 ${jakartaSans.className}`}>Step-by-step</p>
                          <ol className={`mt-2 space-y-2 ${jakartaSans.className}`}>
                            {tool.workflow.map((step, i) => (
                              <li key={i} className="flex gap-2 text-sm font-medium text-slate-600">
                                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-sky-500/10 text-[10px] font-semibold text-sky-700 ring-1 ring-sky-500/20">
                                  {i + 1}
                                </span>
                                {step}
                              </li>
                        ...
                        {tool.prompts.length > 0 ? (
                          <div className="mt-4 space-y-2">
                            <p className={`text-xs font-semibold text-slate-400 ${jakartaSans.className}`}>
                              Copy-paste prompts
                            </p>
                            {tool.prompts.map((prompt) => {
                              const copyKey = `${tool.id}-${prompt.label}`;
                              return (
                                <div key={copyKey} className="rounded-2xl bg-[#f5f5f7] p-3 ring-1 ring-black/[0.04]">
                                  <div className="flex items-start justify-between gap-2">
                                    <p className={`text-xs font-semibold text-slate-700 ${jakartaSans.className}`}>
                                      {prompt.label}
                                    </p>
                                    <button
                                      type="button"
                                      onClick={() => copyPrompt(copyKey, prompt.text)}
                                      className={`inline-flex shrink-0 items-center gap-1 rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-slate-600 ring-1 ring-black/[0.06] transition hover:text-sky-700 ${jakartaSans.className}`}
                                    >
                                      {copiedId === copyKey ? (
                                        <>
                                  ...
                                  <p className={`mt-2 whitespace-pre-wrap text-xs font-medium leading-relaxed text-slate-500 ${jakartaSans.className}`}>
                                    {prompt.text}
                                  </p>
                                </div>
                              );
